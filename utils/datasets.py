...
def create_dataloader(path, imgsz, batch_size, stride, single_cls=False, sample_num=0, hyp=None, augment=False, cache=False, pad=0.0,
...
def img2label_paths(img_paths):
    ...
    return [sb.join(x.rsplit(sa, 1)).rsplit('-color.png', 1)[0] + '-box.txt' for x in img_paths]
...
def load_mosaic(self, index):
    ...
    for i, index in enumerate(indices):
        ...
        # Labels
        labels, segments = self.labels[index].copy(), [] * (self.segments[index]).shape[0]
        if labels.size:
