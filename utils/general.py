...
def labels_to_class_weights(labels, n_attr=16):
    ...
    weights = np.bincount(classes, minlength=n_attr)  # occurrences per class
...
def non_max_suppression(prediction, attribute_matrix, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if classes is not None:
            x = x[(x[:, 5] == torch.tensor(classes, device=x.device)).any(1)]
