...
def accumalate_img(predn, labelsn, iouv, Tp):
    """
    Return correct predictions matrix. Both sets of boxes are in (x1, y1, x2, y2) format.
    Arguments:
        predn: (Array[N, 6]), x1, y1, x2, y2, conf, class
        labelsn: (Array[M, 5]), class, x1, y1, x2, y2
        iouv: iou level as [0.5 0.6 0.7 0.8 0.9]
        Tp: True positives of each class in every iou level
        Pred_t: predicted true boxes in every iou level
    """
    iou = box_iou(labelsn[:, 1:], predn[:, :4])
    for i in range(len(iouv)):
        x = torch.where((iou >= iouv[i]))  # IoU above threshold and classes match
        if x[0].shape[0]:
            index = x[0]
...
def run(data,
    ...
    # Print results
    res_file = open(os.path.join(save_dir, 'results.txt'), 'w')
    ...
    if (verbose or (nc < 50 and not training)) and nc > 1 and len(stats):
        for i, c in enumerate(ap_class):
            ...
            res_file.write((pf % (names[c], seen, nt[c], p[i], r[i], ap50[i], ap[i]))+'\n')
